...
NODE_PARAMS_MAX_RETRIES: Final = 5
NODE_PARAMS_RETRY_DELAYS: Final = (0.4, 0.8, 1.2, 1.8)
METRICS_MAX_RETRIES: Final = 3
METRICS_RETRY_DELAYS: Final = (0.4, 0.8)
...
class TibberLocalBridge:
    ...
    async def _fetch_json_with_retries(self, url: str, context: str, max_attempts: int, retry_delays: tuple[float, ...]):
        last_error = None
        ...
        for attempt in range(1, max_attempts + 1):
            ...
            payload_text = ""
            try:
                async with self.web_session.get(url, auth=self.basic_auth, ssl=False, timeout=10.0) as res:
                    status = res.status
                    payload_text = await res.text()
                    payload_len = len(payload_text)

                    _LOGGER.debug(
                        "%s: attempt %s/%s status=%s payload_len=%s",
                        context,
                        attempt,
                        max_attempts,
                        status,
                    ...
                    if status != 200:
                        ...
                        try:
                            return json.loads(payload_text), attempt
                        except json.JSONDecodeError as exc:
                            last_error = exc
                            _LOGGER.debug(
                                "%s: attempt %s/%s JSON decode failed: %s",
                                context,
                                attempt,
                                max_attempts,
                                exc,
                ...
                last_error = exc
                _LOGGER.debug(
                    "%s: attempt %s/%s request failed: %s - %s",
                    context,
                    attempt,
                    max_attempts,
                    type(exc).__name__,
            ...
            if attempt < max_attempts:
                ...
                _LOGGER.debug(
                    "%s: retrying in %.2fs after attempt %s/%s",
                    context,
                    retry_delay,
                    attempt,
                    max_attempts,
        ...
        if last_error is not None:
            _LOGGER.debug(
                "%s: exhausted retries after %s attempts, last_error=%s - %s",
                context,
                max_attempts,
    ...
    async def detect_com_mode_from_node_param27(self):
        ...
        context = f"detect_com_mode_from_node_param27({self.url_mode})"
        try:
            # {'param_id': 27, 'name': 'meter_mode', 'size': 1, 'type': 'uint8', 'help': '0:IEC 62056-21, 1:Count impressions', 'value': [3]}
            json_resp, attempt_count = await self._fetch_json_with_retries(
                self.url_mode,
                context,
                NODE_PARAMS_MAX_RETRIES,
                NODE_PARAMS_RETRY_DELAYS,
            )
            if json_resp is not None:
                for a_parm_obj in json_resp:
                    if 'param_id' in a_parm_obj and a_parm_obj['param_id'] == 27 or \
                        ...
                        if 'value' in a_parm_obj and len(a_parm_obj['value']) > 0:
                ...
                _LOGGER.debug(
                    "%s: meter_mode=%s after %s attempt(s)",
                    context,
                    self._com_mode,
                    attempt_count,
                )
                ...
                _LOGGER.warning(
                    "%s: meter_mode unavailable after %s attempts; keeping MODE_UNKNOWN for fallback detection",
                    context,
                    attempt_count,
                )
        except Exception as exec:
            _LOGGER.warning(f"{context}: access to bridge failed with exception: {exec}")
        ...
        if not self._metrics_update_is_running:
            ...
            try:
                ...
                if to_wait_till < time.time():
                    context = f"updated_tibber_metrics_if_needed({self.url_metrics})"
                    _LOGGER.debug(f"updated_tibber_metrics_if_needed(): request: {self.url_metrics}")
                    json_resp, attempt_count = await self._fetch_json_with_retries(
                        self.url_metrics,
                        context,
                        METRICS_MAX_RETRIES,
                        METRICS_RETRY_DELAYS,
                    )
                    if json_resp is not None:
                        self._metrics_data = json_resp
                        if log_payload:
                            _LOGGER.debug(f"updated_tibber_metrics_if_needed(): metrics response after {attempt_count} attempt(s): {self._metrics_data}")
                    else:
                        _LOGGER.warning(
                            "updated_tibber_metrics_if_needed(): metrics unavailable after %s attempts",
                            attempt_count,
                        )
